...
Public Class PhotoFile
    ...
    Public Sub RenameAndMove(folder As String)
        ...
        If Not _IsDeleted Then
            ...
            If _File.FullName = proposedFullPath Then Exit Sub

            'find photos with the same hash and delete them
            Dim dupes = From i In Project.Photos Where (Not i.IsDeleted) And SequenceEqualUnsafe(i._Hash, Me._Hash) And i IsNot Me

            Dim q2 = dupes.Count

            For Each d In dupes
                Dim q = (From i In Project.Photos Where i.File.FullName = Me.File.FullName).Count > 1

                d.Delete()
            Next

            '--edge case: Duplicate file which is somehow not in the same project but is still in the same folder--
            'I did a quick analysis of if we should keep the existing or overwrite. I'm opting to overwrite. The file which is already in place has the correct filename but may have been corrupted (maybe this is why the user is running this tool against already existing files?) This means: the hash in the filename of the existing file MAY be correct. But since we just generated the hash for this file, this has IS correct. Long story short, we're overwriting likely correct (but possibly not) files with guaranteed correct files.

            If IO.File.Exists(proposedFullPath.EnsureFilePrepend) And Me._File.FullName.EnsureFilePrepend <> proposedFullPath.EnsureFilePrepend Then

                'a file exists in the destination with the same exact name.
                'check the hash on this file. if it matches the file we are trying to write, delete the one at the destination and use that filename. if the hashes don't match, try [filename](1)
                Dim fileIterator As Integer = 1

                While IO.File.Exists(proposedFullPath.EnsureFilePrepend)

                    Dim tmp As New PhotoFile()
                    tmp._File = New IO.FileInfo(proposedFullPath.EnsureFilePrepend)
                    tmp._refreshMetadata()

                    If SequenceEqualUnsafe(tmp.Hash, Me.Hash) Then
                        IO.File.Delete(proposedFullPath.EnsureFilePrepend)
                        Exit While
                    Else
                        fileIterator += 1
                        proposedFullPath = IO.Path.Combine(IO.Path.GetDirectoryName(proposedFullPath), IO.Path.GetFileNameWithoutExtension(proposedFullPath) & "(" & fileIterator & ")" & IO.Path.GetExtension(proposedFullPath))
                    End If
                End While

            End If

            '--end of edge cases, do the file rename--
